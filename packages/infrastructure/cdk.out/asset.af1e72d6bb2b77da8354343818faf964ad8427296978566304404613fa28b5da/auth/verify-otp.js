...
const handler = async (event) => {
    ...
    try {
        ...
        if (!body.username || !body.code) {
            return createErrorResponse(400, 'MISSING_PARAMETERS', 'Username and verification code are required');
        }
        ...
        if (error.name === 'CodeMismatchException') {
            return createErrorResponse(400, 'INVALID_CODE', 'Invalid verification code. Please check and try again.');
        }
        if (error.name === 'ExpiredCodeException') {
            return createErrorResponse(400, 'EXPIRED_CODE', 'Verification code has expired. Please request a new code.');
        }
        if (error.name === 'NotAuthorizedException') {
            return createErrorResponse(403, 'NOT_AUTHORIZED', 'User cannot be confirmed. Account may already be verified.');
        }
        if (error.name === 'UserNotFoundException') {
            return createErrorResponse(404, 'USER_NOT_FOUND', 'User not found. Please check the username.');
        }
        if (error.name === 'TooManyFailedAttemptsException') {
            return createErrorResponse(429, 'TOO_MANY_ATTEMPTS', 'Too many failed verification attempts. Please try again later.');
        }
        if (error.name === 'LimitExceededException') {
            return createErrorResponse(429, 'LIMIT_EXCEEDED', 'Attempt limit exceeded. Please try again later.');
        }
        // Generic error response
        return createErrorResponse(500, 'INTERNAL_ERROR', 'An error occurred during verification. Please try again.');
    }
};
...
async function findUserIdByUsername(username) {
    try {
        ...
        const emailQuery = new lib_dynamodb_1.QueryCommand({
            TableName: USERS_TABLE_NAME,
        ...
        const phoneQuery = new lib_dynamodb_1.QueryCommand({
            TableName: USERS_TABLE_NAME,

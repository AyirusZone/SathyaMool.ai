...
def convert_floats_to_decimal(obj):
    ...
    DynamoDB does not support float types - all numbers must be Decimal.
    This function recursively traverses dictionaries and lists to convert floats.
...
def repair_pdf_and_extract_text(bucket_name: str, object_key: str) -> Dict[str, Any]:
    ...
    try:
            ...
            if page_text.strip():
                ...
                for line in lines:
                    ...
                    block = {
                        'BlockType': 'LINE',
                        'Text': line,
                        'Confidence': 95,  # Integer for DynamoDB compatibility
                        'Page': page_num + 1
                    }
        ...
        # Note: Use integers instead of floats for DynamoDB compatibility
        result = {
            'text': '\n\n'.join(all_text),
            'raw_text': '\n\n'.join(all_text),
            'blocks': all_blocks,
            ...
            'average_confidence': 95,  # Integer for DynamoDB compatibility
            'detected_language': 'unknown',  # PyMuPDF doesn't detect language
            'low_confidence_count': 0,  # No low confidence blocks with PyMuPDF
            'has_handwritten_text': False,  # PyMuPDF doesn't detect handwriting
            'low_confidence_regions': [],  # No low confidence regions
...
def store_ocr_results(
    ...
    ocr_metadata = convert_floats_to_decimal(ocr_metadata)
    ...
    if ocr_metadata['low_confidence_flag']:
        logger.warning(
            ...
            f"{float(ocr_metadata['average_confidence']):.2f}% "
